...
function replace(target:number,lung:{v:number},soft:{v:number},preferSoft=false){let d=0,t=0;if(preferSoft){t=Math.min(soft.v,target);soft.v-=t;d+=t;t=Math.min(lung.v,target-d);lung.v-=t;return d+t;}t=Math.min(lung.v,target);lung.v-=t;d+=t;t=Math.min(soft.v,target-d);soft.v-=t;return d+t;}
