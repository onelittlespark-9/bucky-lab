...
export async function projectAtlasTissuePaths(args:{patient:Patient;projection:Projection;tube:TubeState;width:number;height:number;geometry:ProjectionGeometry;wholeBody?:boolean;}):Promise<AtlasTissueMaterialPaths|null>{
  ...
  try{
    ...
    const body=blur(project(scene,a,p=>p.system==="integumentary"||p.system==="muscular",camera,renderer,rt,fm,bm,rw,rh),rw,rh,7);
    const muscle=P(p=>p.system==="muscular"&&!/diaphragm/i.test(p.name),8,24,4);
    ...
    const heart=blur(project(scene,a,p=>p.system==="cardiac",camera,renderer,rt,fm,bm,rw,rh),rw,rh,2);
    const vessels=P(p=>p.system==="arterial"||p.system==="venous",.70,3.0,1);
    const diaphragm=blur(project(scene,a,p=>p.system==="muscular"&&/diaphragm/i.test(p.name),camera,renderer,rt,fm,bm,rw,rh),rw,rh,2);
    const liver=P(p=>p.system==="digestive"&&/liver|hepat/i.test(p.name),10,12,2);
    const hollow=P(p=>p.system==="digestive"&&/stomach|colon|intestin|cecum|rect|duoden/i.test(p.name),4,8,1);
    const urinary=P(p=>p.system==="urinary"&&/kidney|renal|bladder/i.test(p.name),5,8,2);
    ...
    for(let i=0;i<body.length;i++){
      const B=body[i]!;if(B<=.003)continue;
      const baseMuscle=Math.min(B*.20,muscle[i]!*.105);
      const fat=Math.min(B*.20,Math.max(B*.035,(B-baseMuscle)*.10));
      const internal=Math.max(0,B-baseMuscle-fat);
      const literalLung=Math.max(left[i]!,right[i]!);
      const lungPresence=clamp01((literalLung-.04)/.40);
      const lungTarget=Math.min(internal*.96,literalLung*lungPresence);
      const lung={v:lungTarget},soft={v:Math.max(0,internal-lungTarget)};
      let mp=baseMuscle,bp=0,brainp=0,airp=0;
      const heartTarget=Math.min(internal*.78,heart[i]!*1.00);
      const vesselTarget=Math.min(internal*.16,vessels[i]!*0.22);
      const diaphragmTarget=Math.min(internal*.26,diaphragm[i]!*0.68);
      let r=replace(heartTarget,lung,soft);mp+=r*.38;bp+=r*.62;
      r=replace(vesselTarget,lung,soft);bp+=r;
      r=replace(Math.min(internal*.11,airway[i]!*.72),lung,soft);airp+=r;
      r=replace(diaphragmTarget,lung,soft);mp+=r*.95;bp+=r*.05;
      const br=Math.min(soft.v,brain[i]!*0.92);soft.v-=br;brainp+=br;
      const lv=Math.min(soft.v,Math.min(internal*.48,liver[i]!*0.46));soft.v-=lv;mp+=lv*.52;bp+=lv*.48;
      const renal=Math.min(soft.v,Math.min(internal*.18,urinary[i]!*0.32));soft.v-=renal;mp+=renal*.42;bp+=renal*.58;
      const gas=Math.min(soft.v*.30,hollow[i]!*0.12);soft.v-=gas;airp+=gas;
