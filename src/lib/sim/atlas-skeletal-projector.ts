...
function resetAtlas(a:LoadedAtlas,p:Patient){
  a.root.position.set(0,0,0);a.root.rotation.set(0,0,0);a.root.quaternion.identity();a.root.scale.setScalar((p.heightCm/100)/ATLAS_HEIGHT_M);
  for(const q of a.parts){const c=q.center;q.mesh.position.set(c[0],c[1],c[2]);q.mesh.quaternion.identity();q.mesh.scale.setScalar(1);q.mesh.visible=true;}
}
...
function lookupExcess(t:Float32Array,p:number){const q=Math.max(0,Math.min(MAX_BONE_PATH_CM,p))/LOOKUP_STEP_CM,i0=Math.floor(q),i1=Math.min(t.length-1,i0+1),f=q-i0;return t[i0]!*(1-f)+t[i1]!*f;}
...
export async function projectAtlasSkeletalOD(args:{patient:Patient;projection:Projection;pose?:SimPose;tube:TubeState;exposureKvp:number;width:number;height:number;geometry:ProjectionGeometry;wholeBody?:boolean;}):Promise<Float32Array|null>{
  ...
  try{
    ...
    let target=centre.clone();if(!wholeBody){const targetYcm=projection.cr.y*patient.heightCm/170,targetXcm=projection.cr.x;target=new THREE.Vector3(targetXcm/100,patient.heightCm/100-targetYcm/100,0);}
