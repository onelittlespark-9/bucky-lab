...
function materialPaths(region:BoneRegion,name:string,path:number,outerDiameter=path){const n=name.toLowerCase();let shell=.14,trabFrac=.40;if(region==="humerus"||region==="femur"){shell=.24;trabFrac=.085;}else if(region==="forearm"||region==="lowerleg"){shell=.17;trabFrac=.075;}else if(region==="rib"){shell=.040;trabFrac=.20;}else if(region==="axial"){shell=.050;trabFrac=.34;}else if(region==="pelvis"){shell=n.includes("sacrum")?.060:.075;trabFrac=.46;}else if(region==="scapula"){shell=.045;trabFrac=.36;}else if(region==="clavicle"){shell=.105;trabFrac=.20;}else if(region==="hand"||region==="foot"){shell=.065;trabFrac=.30;}else if(region==="patella"){shell=.060;trabFrac=.46;}else if(region==="skull"){shell=n.includes("mandible")?.12:.075;trabFrac=.30;}const shellGeometry=isLongBone(region)?shellChord(path,outerDiameter,shell):{cortical:Math.min(path,shell*2),interior:Math.max(0,path-Math.min(path,shell*2))},cortical=shellGeometry.cortical,interior=shellGeometry.interior,canalWeight=isLongBone(region)?Math.min(1,interior/.80):0,centralWeight=(region==="rib"||region==="axial")?Math.min(1,interior/.75):0;let effectiveTrabFrac=trabFrac*(1-.72*canalWeight);effectiveTrabFrac*=1-.28*centralWeight;const trabecular=interior*effectiveTrabFrac,marrow=interior-trabecular;return{cortical,trabecular,marrow};}
