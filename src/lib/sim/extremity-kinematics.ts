...
export interface ExtremityPlacement { target: ExtremityTarget; side: Side | null; onDetector: boolean; detectorTilt: number; }
...
/** Apply a sensible starting pose for an extremity presented on a detector. */
export function poseForExtremityPlacement(projectionId: string, placement: PlacementMode, buckyTilt: number, base: SimPose): SimPose {
  ...
  switch (intent.target) {
    case "hand":
    case "wrist":
      return { ...base, armRaise: 0.5, elbowFlex: 0 };
    case "elbow":
