...
async function canonicalMaps(args:{patient:Patient;projection:Projection;tube:TubeState;exposureKvp:number;geometry:ProjectionGeometry;}){
  const{patient,projection,tube,exposureKvp,geometry}=args,baseWidth=640,baseHeight=1920;
  const canonicalTube:TubeState={...tube,crX:0,crY:CANONICAL_CR_Y_CM,collimationW:CANONICAL_W_CM,collimationH:CANONICAL_H_CM};
  const key=[patient.id,patient.heightCm,patient.weightKg,patient.habitus,exposureKvp,geometry.magnification.toFixed(5)].join("|");
  let pending=CACHE.get(key);
  if(!pending){pending=Promise.all([
    projectAtlasSkeletalOD({patient,projection,tube:canonicalTube,exposureKvp,width:baseWidth,height:baseHeight,geometry,wholeBody:true}),
    projectAtlasTissueOD({patient,projection,tube:canonicalTube,exposureKvp,width:baseWidth,height:baseHeight,geometry,wholeBody:true}),
  ]).then(([bone,tissue])=>({bone,tissue,width:baseWidth,height:baseHeight}));CACHE.set(key,pending);}
  return pending;
}
...
export async function canonicalAtlasProjection(args:{patient:Patient;projection:Projection;tube:TubeState;exposureKvp:number;width:number;height:number;geometry:ProjectionGeometry;}){
  const{patient,projection,tube,exposureKvp,width,height,geometry}=args,maps=await canonicalMaps({patient,projection,tube,exposureKvp,geometry});
  return{bone:cropCanonical(maps.bone,maps.width,maps.height,tube,geometry,width,height),tissue:cropCanonical(maps.tissue,maps.width,maps.height,tube,geometry,width,height)};
}
