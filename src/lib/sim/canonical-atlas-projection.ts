...
export async function canonicalAtlasProjection(args:{patient:Patient;projection:Projection;tube:TubeState;exposureKvp:number;width:number;height:number;geometry:ProjectionGeometry;}){
  ...
  // This stage now performs only atlas cache/crop/resampling and kVp scaling.
  // No painted lung darkening, cortical edge enhancement, bone fill or local
  // contrast manipulation is permitted here: those effects must emerge from
  // material path lengths and the primary Beer-Lambert calculation.
  const tissue=suppressProceduralFallback(cropCanonical(maps.tissue,maps.width,maps.height,framedTube,geometry,width,height,tissueScale));
  const bone=cropCanonical(maps.bone,maps.width,maps.height,framedTube,geometry,width,height,boneScale);
