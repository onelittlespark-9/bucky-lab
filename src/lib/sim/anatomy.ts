...
function extremityBase(p:Paths,c:SampleCtx,x:number,y:number){const {patient,seed}=c;p.soft+=Math.max(0,patient.thickness.limb)*.35;p.fat+=patient.habitus==="hypersthenic" ? .5 : .25;p.soft+=(fbm(x*1.8,y*1.8,seed)-.5)*.12}
