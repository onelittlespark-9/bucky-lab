...
export async function wholeBodyAtlasTissueOpticalDensity(args: {
  ...
  try {
    ...
    const skinSoft = blurMap(skin, rw, rh, 9);
    ...
    const skinCoverage = blurMap(skinCoverageRaw, rw, rh, 8);
    ...
    const heart = blurMap(heartRaw, rw, rh, 5);
    const heartCoverage = blurMap(heartCoverageRaw, rw, rh, 3);
    const centralVessels = blurMap(centralVesselsRaw, rw, rh, 7);
    const allVessels = blurMap(allVesselsRaw, rw, rh, 10);
    const diaphragm = blurMap(diaphragmRaw, rw, rh, 4);
    const digestive = blurMap(digestiveRaw, rw, rh, 6);
    const urinary = blurMap(urinaryRaw, rw, rh, 5);
    const liver = blurMap(liverRaw, rw, rh, 6);
    const spleen = blurMap(spleenRaw, rw, rh, 5);
    const kidneys = blurMap(kidneysRaw, rw, rh, 5);
    ...
    for (let i = 0; i < low.length; i++) {
      ...
      const boundaryTaper = Math.pow(smoothstep(0.025, 0.92, skinCoverage[i]!), 0.98);
      const mixedDepth = skin[i]! * 0.14 + skinSoft[i]! * 0.86;
      const tangentLimited = Math.min(mixedDepth, skinSoft[i]! * 1.05 + 0.020);
      ...
      const atlasThoraxMask = clamp01((respiratoryMask[i]! - 0.008) / 0.50);
      ...
      const envelopeAssist = bodySupportedEnvelope * (useEnvelopeFallback ? 1.0 : 0.20);
      const thoraxMask = clamp01(Math.max(atlasThoraxMask, envelopeAssist));
      const bilateralGap = smoothstep(0.050, 0.150, Math.abs(xNorm));
      const lungSilhouette = thoraxMask * (0.02 + 0.98 * bilateralGap);
      const lungCore = Math.pow(clamp01(lungSilhouette), 0.52);
      const measuredLungPath = Math.min(internalCapacity * 0.985, lungDepth[i]! * 0.96);
      const atlasSupportedPath = internalCapacity * (0.70 + 0.27 * lungCore) * atlasThoraxMask;
      const envelopeLungPath = internalCapacity * 0.955 * lungCore;
      const lungCandidate = Math.min(
        internalCapacity * 0.985,
        Math.max(measuredLungPath, atlasSupportedPath, envelopeLungPath * (useEnvelopeFallback ? 1.0 : 0.18)),
      );

      const heartShape = clamp01((heartCoverage[i]! - 0.018) / 0.58);
      const mediastinalCore = Math.exp(-Math.pow((xNorm + 0.012) / 0.108, 2)) * thoraxMask;
      const heartPath = Math.min(internalCapacity * 0.74, Math.max(heart[i]! * 1.18, internalCapacity * 0.54 * heartShape));
      const centralSoftPath = internalCapacity * 0.25 * mediastinalCore;
      const centralVesselPath = Math.min(internalCapacity * 0.040, centralVessels[i]! * 0.12);
      const peripheralVesselPath = Math.min(internalCapacity * 0.014, allVessels[i]! * 0.008 * lungCore);
      const diaphragmPath = Math.min(internalCapacity * 0.29, diaphragm[i]! * 1.08);
      const mediastinalDisplacement = Math.min(
        lungCandidate,
        heartPath * 0.72 + centralSoftPath * 0.86 + centralVesselPath * 0.38 + diaphragmPath * 0.48,
      );
      ...
      const bloodPath = Math.min(afterBrain, heartPath * 0.92 + centralVesselPath * 0.52 + peripheralVesselPath * 0.30);
      ...
      const lungInterstitial = lungPath * 0.024;
      ...
      const od =
        ...
        diaphragmPath * Math.max(0, muMuscle - muLung) * 0.58 +
        Math.min(lungPath, centralVesselPath * 0.34 + peripheralVesselPath * 0.30) * Math.max(0, muBlood - muLung) * 0.24;
