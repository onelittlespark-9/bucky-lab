...
function projectParts(scene:THREE.Scene,root:THREE.Group,parts:LoadedPart[],system:string,camera:THREE.OrthographicCamera,renderer:THREE.WebGLRenderer,target:THREE.WebGLRenderTarget,fm:THREE.ShaderMaterial,bm:THREE.ShaderMaterial,w:number,h:number,perPartCap:number,totalCap:number){const selected=parts.filter(p=>p.system===system),sum=new Float32Array(w*h);for(const part of selected){const t=projectVisible(scene,root,parts,p=>p===part,camera,renderer,target,fm,bm,w,h);for(let i=0;i<sum.length;i++)sum[i]=Math.min(totalCap,sum[i]!+Math.min(perPartCap,t[i]!));}return sum;}
...
export async function wholeBodyAtlasTissueOpticalDensity(args:{patient:Patient;tube:TubeState;exposureKvp:number;width:number;height:number;geometry:ProjectionGeometry;}):Promise<Float32Array|null>{
 ...
 try{
  ...
  const rw=Math.min(384,Math.max(192,width)),rh=Math.min(768,Math.max(384,height));
  ...
  const muscle=projectParts(scene,atlas.root,atlas.parts,"muscular",camera,renderer,target,fm,bm,rw,rh,5.5,18);
  const lung=projectParts(scene,atlas.root,atlas.parts,"respiratory",camera,renderer,target,fm,bm,rw,rh,12,24);
  const heart=projectParts(scene,atlas.root,atlas.parts,"cardiac",camera,renderer,target,fm,bm,rw,rh,10,12);
  const digestive=projectParts(scene,atlas.root,atlas.parts,"digestive",camera,renderer,target,fm,bm,rw,rh,9,22);
  const urinary=projectParts(scene,atlas.root,atlas.parts,"urinary",camera,renderer,target,fm,bm,rw,rh,7,12);
  const low=new Float32Array(rw*rh),muSoft=mu(32,exposureKvp),muLung=mu(-780,exposureKvp),muOrgan=mu(58,exposureKvp),muMuscle=mu(48,exposureKvp);
  for(let i=0;i<low.length;i++){const body=skin[i]!>0?skin[i]!:Math.min(32,muscle[i]!*1.15);if(body<=0)continue;const baseline=Math.min(42,body)*muSoft*.052;const lungReplace=Math.min(body*.72,lung[i]!*1.08);const aeration=lungReplace*(muLung-muSoft)*.052;const muscleMod=Math.min(body,muscle[i]!)*Math.max(0,muMuscle-muSoft)*.018;const heartAdd=Math.min(body,heart[i]!)*Math.max(0,muOrgan-muSoft)*.052;const digestAdd=Math.min(body,digestive[i]!)*Math.max(0,muOrgan-muSoft)*.030;const urinaryAdd=Math.min(body,urinary[i]!)*Math.max(0,muOrgan-muSoft)*.028;low[i]=Math.max(.0015,baseline+aeration+muscleMod+heartAdd+digestAdd+urinaryAdd);}
