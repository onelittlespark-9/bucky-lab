...
async function loadAtlas():Promise<TissueAtlas>{
 ...
 cache=(async()=>{
  ...
  for(const part of selected){
   ...
   g.translate(-c.x,-c.y,-c.z);const mesh=new THREE.Mesh(g);mesh.position.copy(c);mesh.name=part.name;root.add(mesh);parts.push({mesh,system:part.system,name:part.name});
  }
  return{root,parts};
 })();return cache;
}
...
function projectVisible(scene:THREE.Scene,root:THREE.Group,parts:LoadedPart[],visible:(p:LoadedPart)=>boolean,camera:THREE.OrthographicCamera,renderer:THREE.WebGLRenderer,target:THREE.WebGLRenderTarget,fm:THREE.ShaderMaterial,bm:THREE.ShaderMaterial,w:number,h:number){
 for(const p of parts)p.mesh.visible=visible(p);root.updateMatrixWorld(true);
 ...
 for(let i=0;i<out.length;i++){const j=i*4,a=unpack(front,j),z=unpack(back,j);if(a>=.9999||z>=.9999)continue;const cm=Math.abs(z-a)*range*100;if(cm>0&&cm<55)out[i]=cm;}
 return out;
}
...
export async function wholeBodyAtlasTissueOpticalDensity(args:{patient:Patient;tube:TubeState;exposureKvp:number;width:number;height:number;geometry:ProjectionGeometry;}):Promise<Float32Array|null>{
 ...
 try{
  ...
  const rw=Math.min(384,Math.max(192,width)),rh=Math.min(768,Math.max(384,height)),camera=new THREE.OrthographicCamera(0,1,1,0,.01,5);
  camera.left=-tube.collimationW/geometry.magnification/200;camera.right=tube.collimationW/geometry.magnification/200;camera.top=tube.collimationH/geometry.magnification/200;camera.bottom=-tube.collimationH/geometry.magnification/200;camera.position.set(centre.x,centre.y,centre.z+2.5);camera.lookAt(centre);camera.updateProjectionMatrix();
  ...
  const target=new THREE.WebGLRenderTarget(rw,rh,{format:THREE.RGBAFormat,type:THREE.UnsignedByteType,depthBuffer:true,stencilBuffer:false}),vs=`varying float vDepth;void main(){vec4 mv=modelViewMatrix*vec4(position,1.0);gl_Position=projectionMatrix*mv;vDepth=gl_Position.z/gl_Position.w*.5+.5;}`,fs=`varying float vDepth;vec3 packDepth24(float v){v=clamp(v,0.0,0.999999);vec3 enc=fract(v*vec3(1.0,255.0,65025.0));enc-=enc.yzz*vec3(1.0/255.0,1.0/255.0,0.0);return enc;}void main(){gl_FragColor=vec4(packDepth24(vDepth),1.0);}`,fm=new THREE.ShaderMaterial({vertexShader:vs,fragmentShader:fs,side:THREE.FrontSide,depthTest:true,depthWrite:true}),bm=new THREE.ShaderMaterial({vertexShader:vs,fragmentShader:fs,side:THREE.BackSide,depthTest:true,depthWrite:true});
  const skin=projectVisible(scene,atlas.root,atlas.parts,p=>p.system==="integumentary",camera,renderer,target,fm,bm,rw,rh),muscleRaw=projectVisible(scene,atlas.root,atlas.parts,p=>p.system==="muscular",camera,renderer,target,fm,bm,rw,rh);
  let lung=projectVisible(scene,atlas.root,atlas.parts,p=>p.system==="respiratory",camera,renderer,target,fm,bm,rw,rh);
  if(maxValue(lung)<.8){const fallback=projectParts(scene,atlas.root,atlas.parts,p=>p.system==="respiratory",camera,renderer,target,fm,bm,rw,rh,15,31);if(maxValue(fallback)>maxValue(lung))lung=fallback;}
  const heart=projectParts(scene,atlas.root,atlas.parts,p=>p.system==="cardiac",camera,renderer,target,fm,bm,rw,rh,10,12),digestive=projectParts(scene,atlas.root,atlas.parts,p=>p.system==="digestive",camera,renderer,target,fm,bm,rw,rh,9,22),urinary=projectParts(scene,atlas.root,atlas.parts,p=>p.system==="urinary",camera,renderer,target,fm,bm,rw,rh,7,12),centralVessels=projectParts(scene,atlas.root,atlas.parts,p=>(p.system==="arterial"||p.system==="venous")&&/(pulmonary|aorta|aortic|vena cava|caval|brachiocephalic|subclavian)/i.test(p.name),camera,renderer,target,fm,bm,rw,rh,2.8,7.5),diaphragm=projectParts(scene,atlas.root,atlas.parts,p=>p.system==="muscular"&&/diaphragm/i.test(p.name),camera,renderer,target,fm,bm,rw,rh,2.5,4),liver=projectParts(scene,atlas.root,atlas.parts,p=>/liver|hepatic/i.test(p.name),camera,renderer,target,fm,bm,rw,rh,12,14),spleen=projectParts(scene,atlas.root,atlas.parts,p=>/spleen|splenic/i.test(p.name),camera,renderer,target,fm,bm,rw,rh,7,8),kidneys=projectParts(scene,atlas.root,atlas.parts,p=>p.system==="urinary"&&/kidney|renal/i.test(p.name),camera,renderer,target,fm,bm,rw,rh,7,12),brain=projectParts(scene,atlas.root,atlas.parts,p=>p.system==="nervous"&&/brain|cerebr|encephal/i.test(p.name),camera,renderer,target,fm,bm,rw,rh,14,18),low=new Float32Array(rw*rh),muSoft=mu(30,exposureKvp),muLung=mu(-850,exposureKvp),muOrgan=mu(62,exposureKvp),muMuscle=mu(48,exposureKvp),muBlood=mu(65,exposureKvp),muBrain=mu(38,exposureKvp),scale=.052;
  for(let i=0;i<low.length;i++){
   const skinDepth=skin[i]!,muscle=Math.min(skinDepth>0?skinDepth*.78:18,muscleRaw[i]!),body=skinDepth>0?skinDepth:Math.min(32,muscle*1.15);if(body<=0)continue;
   const baseline=Math.min(42,body)*muSoft*scale,lungReplace=Math.min(body*.86,lung[i]!*1.30),aeration=lungReplace*(muLung-muSoft)*scale*1.10,muscleMod=Math.min(body,muscle)*Math.max(0,muMuscle-muSoft)*.012,heartDepth=Math.min(body*.58,heart[i]!),vesselDepth=Math.min(body*.22,centralVessels[i]!),diaphragmDepth=Math.min(body*.16,diaphragm[i]!),heartOverlap=Math.min(lungReplace,heartDepth*.98),vesselOverlap=Math.min(Math.max(0,lungReplace-heartOverlap),vesselDepth*.82),diaphragmOverlap=Math.min(Math.max(0,lungReplace-heartOverlap-vesselOverlap),diaphragmDepth*.78),heartRestore=heartOverlap*(muSoft-muLung)*scale*1.02,vesselRestore=vesselOverlap*(muSoft-muLung)*scale,diaphragmRestore=diaphragmOverlap*(muSoft-muLung)*scale*.96,heartDensity=heartDepth*Math.max(0,muOrgan-muSoft)*.074,vesselDensity=vesselDepth*Math.max(0,muBlood-muSoft)*.085,diaphragmDensity=diaphragmDepth*Math.max(0,muMuscle-muSoft)*.052,digestAdd=Math.min(body,digestive[i]!)*Math.max(0,muOrgan-muSoft)*.022,urinaryAdd=Math.min(body,urinary[i]!)*Math.max(0,muOrgan-muSoft)*.020,liverAdd=Math.min(body*.68,liver[i]!)*Math.max(0,muOrgan-muSoft)*.067,spleenAdd=Math.min(body*.38,spleen[i]!)*Math.max(0,muOrgan-muSoft)*.055,kidneyAdd=Math.min(body*.32,kidneys[i]!)*Math.max(0,muOrgan-muSoft)*.050,brainAdd=Math.min(body*.70,brain[i]!)*Math.max(0,muBrain-muSoft)*.035;
  ...
  if(maxValue(lung)<.8)console.warn("[Bucky Lab] Respiratory atlas projection is unexpectedly shallow; whole-body lung contrast may be degraded.");
  const softened=blurMap(low,rw,rh);for(let i=0;i<low.length;i++)low[i]=low[i]*.80+softened[i]*.20;
  for(const p of atlas.parts)p.mesh.visible=true;scene.overrideMaterial=null;renderer.dispose();target.dispose();fm.dispose();bm.dispose();
  const full=new Float32Array(width*height);for(let y=0;y<height;y++){const sy=(1-y/Math.max(1,height-1))*(rh-1);for(let x=0;x<width;x++){const sx=x/Math.max(1,width-1)*(rw-1);full[y*width+x]=sampleBilinear(low,rw,rh,sx,sy);}}
  return full;
