...
export type AnatomyKind = "full-body-ap" | "torso-ap" | "torso-lat" | "hand-pa" | "wrist-pa" | "elbow-ap" | "shoulder-ap" | "knee-ap" | "knee-lat" | "foot-dp" | "ankle-ap" | "skull-lat" | "cspine-lat";
export type Region = "Whole body" | "Thorax" | "Abdomen" | "Pelvis & hips" | "Spine" | "Upper limb" | "Lower limb" | "Skull";
...
export type ArmSide = "left" | "right" | null;
export interface Patient { id:string; name:string; age:number; sex:Sex; habitus:Habitus; heightCm:number; weightKg:number; skin:string; hair:string; gown:string; thickness:{chest:number;abdomen:number;pelvis:number;cspine:number;lumbar:number;skull:number;extremity:number;shoulder:number;knee:number}; morph:{torsoWidth:number;torsoDepth:number;torsoLength:number;limb:number;shoulder:number;hip:number;abdomen:number;kyphosis:number;breast:number}; notes:string; }
export interface Landmark { id:string; label:string; y:number; x:number; }
export interface Projection { id:string; name:string; shortName:string; region:Region; anatomy:AnatomyKind; setup:Setup; recumbency:Recumbency; laterality?:"left"|"right"; irW:number; irH:number; grid:boolean; sidCm:number; tubeAngle:number; kvp:number; mas:number; collimationW:number; collimationH:number; cr:{x:number;y:number}; landmarkId:string; centring:string; position:string; beam:string; collimation:string; criteria:string[]; include:string[]; respiration?:Breath; referenceImage?:string; photoBounds?:{x0:number;y0:number;x1:number;y1:number}; }
export interface SimPose { recumbency:Recumbency; rotationY:number; oblique:number; chinUp:number; shoulderRoll:number; armRaise:number; elbowFlex:number; armSide:ArmSide; armRotation:number; forearmRotation:number; kneeFlex:number; hipInternal:number; breath:Breath; }
