...
export function scoreExposure(args: {
  ...
  const dy = Math.abs(tube.crY - targetY);
  const dx = Math.abs(tube.crX - targetX);
  ...
  scores.push({
    ...
    detail: dist < 1.6 ? `CR within ${dist.toFixed(1)} cm of ${projection.centring}` : `CR is ${dist.toFixed(1)} cm from the handbook point (${projection.centring})`,
  });
  ...
  if (clipped) { colGrade = "repeat"; colDetail = "Shutters too tight — required anatomy is likely clipped."; }
  ...
  if (projection.id === "ap-pelvis" || projection.id === "ap-hip") {
    ...
    scores.push({ id: "feet", label: "Internal rotation of hips", weight: 0.8, grade: hip, detail: pose.hipInternal >= 12 ? "Femoral necks parallel to the IR." : "Internally rotate both feet 15–20° so the necks are not foreshortened." });
  }
  if (projection.id === "lat-knee") {
    const kf = Math.abs(pose.kneeFlex - 25);
    scores.push({ id: "flex", label: "Knee flexion", weight: 0.6, grade: gradeFromError(kf, 8, 18), detail: `Flexed ${pose.kneeFlex.toFixed(0)}° (handbook 20–30°).` });
  }
