...
async function loadAtlas():Promise<AtlasScene>{
  ...
  const root=new THREE.Group();root.name="Bucky-Lab-atlas-radiograph-skeleton";const meshes:THREE.Mesh[]=[];
  ...
  for(const {part,buffer} of [...chunks.values()].flat()){
    ...
    geometry.translate(-center.x,-center.y,-center.z);geometry.computeBoundingSphere();
    const mesh=new THREE.Mesh(geometry);mesh.name=`Atlas radiograph ${part.name}`;
    mesh.userData.atlasRegion=regionFor(part.name);mesh.userData.atlasSide=sideFor(part.bounds);mesh.userData.atlasCenter=[center.x,center.y,center.z] as V3;
...
function articulate(root:THREE.Group,meshes:THREE.Mesh[],pose:SimPose,patient:Patient,placement:PlacementMode,projectionId:string){
  ...
  root.scale.setScalar(scale);const yAxis=new THREE.Vector3(0,1,0);const local=(p:V3)=>new THREE.Vector3(p[0]/scale,p[1]/scale,p[2]/scale);const mid=(a:THREE.Vector3,b:THREE.Vector3)=>a.clone().add(b).multiplyScalar(.5);
  for(const object of meshes){
    const region=object.userData.atlasRegion as string,side=object.userData.atlasSide as -1|1,centre=object.userData.atlasCenter as V3;
    object.position.copy(local(centre));object.rotation.set(0,0,0);object.scale.setScalar(1);
    if(region==="rib"||region==="axial"||region==="skull")continue;
    const arm=kin.arms[side<0?0:1],leg=kin.legs[side<0?0:1];const shoulder=local(arm.shoulder),elbow=local(arm.elbow),wrist=local(arm.wrist),hand=local(arm.hand),hip=local(leg.hip),knee=local(leg.knee),ankle=local(leg.ankle),foot=local(leg.foot);
    ...
    if(region==="pelvis"){object.position.set(0,.47*H/scale,0);continue;}
...
/** HU is a CT calibration scale, used here as a reproducible tissue-equivalent density proxy for projection radiography. */
...
function muFromHU(hu:number,kvp:number):number{
  ...
  if(hu<=-1000)return muAir;if(hu<=0)return muWater+(hu/1000)*(muWater-muAir);if(hu<=1000)return muWater+(hu/1000)*(muBone1000-muWater);
  return muBone1000+Math.min(1000,hu-1000)*.00018*energyScale;
}
function unpackDepth(r:number):number{return r/255;}

function renderRegionThickness(scene:THREE.Scene,root:THREE.Group,meshes:THREE.Mesh[],region:string,camera:THREE.OrthographicCamera,renderer:THREE.WebGLRenderer,renderTarget:THREE.WebGLRenderTarget,frontMaterial:THREE.ShaderMaterial,backMaterial:THREE.ShaderMaterial,rw:number,rh:number):Float32Array{
  for(const mesh of meshes)mesh.visible=mesh.userData.atlasRegion===region;
  const front=new Uint8Array(rw*rh*4),back=new Uint8Array(rw*rh*4);root.updateMatrixWorld(true);
  scene.overrideMaterial=frontMaterial;renderer.setRenderTarget(renderTarget);renderer.clear(true,true,true);renderer.render(scene,camera);renderer.readRenderTargetPixels(renderTarget,0,0,rw,rh,front);
  scene.overrideMaterial=backMaterial;renderer.clear(true,true,true);renderer.render(scene,camera);renderer.readRenderTargetPixels(renderTarget,0,0,rw,rh,back);
  const result=new Float32Array(rw*rh),rangeCm=camera.far-camera.near;
  for(let i=0;i<result.length;i++){const j=i*4,f=unpackDepth(front[j]!),b=unpackDepth(back[j]!);if(f>=.9999||b>=.9999||b<=f)continue;result[i]=Math.max(0,(b-f)*rangeCm*100);}
  return result;
}
...
export async function atlasBoneOpticalDensity(args:{patient:Patient;projection:Projection;pose:SimPose;tube:TubeState;exposureKvp:number;width:number;height:number;geometry:ProjectionGeometry;}):Promise<Float32Array|null>{
  ...
  try{
    const atlas=await loadAtlas();const placement:PlacementMode=projection.setup==="table"||projection.setup==="tabletop"?"table":"upright-bucky";articulate(atlas.root,atlas.meshes,pose,patient,placement,projection.id);
    ...
    const camera=new THREE.OrthographicCamera(0,1,1,0,.01,5);const halfW=tube.collimationW/geometry.magnification/200,halfH=tube.collimationH/geometry.magnification/200;
    camera.left=-halfW;camera.right=halfW;camera.top=halfH;camera.bottom=-halfH;camera.position.copy(lateral?new THREE.Vector3(target.x+2.5,target.y,target.z):new THREE.Vector3(target.x,target.y,target.z+2.5));camera.lookAt(target);camera.updateProjectionMatrix();
    const renderer=new THREE.WebGLRenderer({antialias:false,alpha:false,preserveDrawingBuffer:false});renderer.setSize(rw,rh,false);renderer.setPixelRatio(1);renderer.outputColorSpace=THREE.LinearSRGBColorSpace;
    const scene=new THREE.Scene();scene.background=new THREE.Color(0x000000);scene.add(atlas.root);
    ...
    const regions=["axial","rib","scapula","clavicle","skull","pelvis","femur","lowerleg","humerus","forearm","hand","foot"],low=new Float32Array(rw*rh);
    for(const region of regions){if(!atlas.meshes.some(m=>m.userData.atlasRegion===region))continue;const thickness=renderRegionThickness(scene,atlas.root,atlas.meshes,region,camera,renderer,renderTarget,frontMaterial,backMaterial,rw,rh);const hu=materialHU(region,projection),muTrab=muFromHU(hu.trabecular,exposureKvp),muCort=muFromHU(hu.cortical,exposureKvp);
    ...
    for(const mesh of atlas.meshes)mesh.visible=true;scene.overrideMaterial=null;renderer.dispose();renderTarget.dispose();frontMaterial.dispose();backMaterial.dispose();
