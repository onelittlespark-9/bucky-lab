...
function focusForProjection(region:string, anatomy:string): { target:FocusPoint; distance:number } {
  ...
  if (region === "Lower limb") return {target:[0,.45,0],distance:1.35};
  if (region === "Pelvis & hips") return {target:[0,.72,0],distance:1.5};
  if (region === "Spine") return {target:[0,1.05,0],distance:1.55};
  if (region === "Abdomen") return {target:[0,.92,0],distance:1.55};
...
export function RoomQuickControls(){
  ...
  return <>
    <OrbitControls makeDefault enablePan minPolarAngle={.2} maxPolarAngle={Math.PI/2.05} minDistance={.75} maxDistance={6} target={[0,.9,0]} />
