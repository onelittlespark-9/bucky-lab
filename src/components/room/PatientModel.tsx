...
export function PatientModel() {
 const patientId = useSim(s => s.patientId), pose = useSim(s => s.pose), equipment = useSim(s => s.equipment), showLandmarks = useSim(s => s.showLandmarks), setLandmarkCR = useSim(s => s.setLandmarkCR);
 const patient = patientById(patientId), H = patient.heightCm / 100, s = H / 1.7, m = patient.morph, skin = patient.skin;
 const bodyWidth = .16 * m.torsoWidth * s, chestDepth = .12 * m.torsoDepth * s, shoulderWidth = .205 * m.shoulder * s, limb = .043 * m.limb * s;
 const yaw = pose.rotationY * Math.PI / 180, oblique = pose.oblique * Math.PI / 180, kyphosis = m.kyphosis * .22;
 ...
 return <group position={groupPos} rotation={groupRot}>
  ...
  <group rotation={[kyphosis, oblique, 0]}>
   ...
   <Ellipsoid position={[0, .67 * H, 0]} scale={[bodyGeometry.chest[0] * 1.035, bodyGeometry.chest[1] * .42, bodyGeometry.chest[2] * 1.035]} color={patient.gown} castShadow={false} /><Ellipsoid position={[0, Y.neck, 0]} scale={[.055 * m.shoulder * s, .075 * s, .06 * s]} color={skin} /><Head position={[0, Y.head, 0]} skin={skin} hair={patient.hair} chin={pose.chinUp * .16} scale={s} />
   {([-1, 1] as const).map(side => { const shoulder: V3 = [side * shoulderWidth, Y.shoulder, 0]; let upper: V3, elbowPoint: V3, wrist: V3; if (lateralChest && raise > .35) { upper = [side * (shoulderWidth + .03 * s), Y.shoulder + .12 * H * raise, .02 * s]; elbowPoint = [side * (shoulderWidth + .015 * s), Y.shoulder + .22 * H * raise, .035 * s]; wrist = [side * .075 * s, Y.shoulder + .31 * H * raise, .04 * s]; } else if (paChest && pose.shoulderRoll > .55) { upper = [side * (shoulderWidth + .02 * s), Y.shoulder - .045 * s, 0]; elbowPoint = [side * (shoulderWidth + .015 * s), Y.shoulder - .12 * s, .055 * s]; wrist = [side * (bodyWidth * .52), Y.waist + .02 * s, .07 * s]; } else { upper = [side * (shoulderWidth + .055 * m.limb * s), Y.shoulder + .08 * H * raise, 0]; elbowPoint = [upper[0] + side * .005 * s, upper[1] - .16 * s * Math.cos(elbow), .02 * s * Math.sin(elbow)]; wrist = [elbowPoint[0] + side * .012 * s, elbowPoint[1] - .16 * s * Math.cos(elbow), .04 * s * Math.sin(elbow)]; } return <group key={side}><Ellipsoid position={shoulder} scale={[.065 * m.shoulder * s, .065 * s, .065 * s]} color={skin} /><Segment a={shoulder} b={upper} radius={limb * 1.28} color={skin} /><Ellipsoid position={elbowPoint} scale={[limb * 1.28, limb * 1.18, limb * 1.2]} color={skin} /><Segment a={upper} b={elbowPoint} radius={limb} color={skin} /><Segment a={elbowPoint} b={wrist} radius={limb * .86} color={skin} /><Hand position={wrist} side={side} color={skin} scale={m.limb * s} /></group>; })}
   {([-1, 1] as const).map(side => { const hip: V3 = [side * hipGap, Y.pelvis - .01 * s, 0], thigh: V3 = [side * (hipGap + .008 * s), Y.knee + .12 * H, side * .008 * s * Math.sin(hipInt)], knee: V3 = [side * (hipGap + .006 * s), Y.knee, side * .012 * s], calf: V3 = [side * (hipGap + .006 * s), Y.ankle + .12 * H, side * .008 * s], ankle: V3 = [side * (hipGap + .006 * s), Y.ankle, side * .01 * s]; return <group key={side}><Ellipsoid position={hip} scale={[limb * 1.75, limb * 1.8, limb * 1.7]} color={skin} /><Segment a={hip} b={thigh} radius={limb * 1.45} color={skin} /><Ellipsoid position={knee} scale={[limb * 1.38, limb * 1.28, limb * 1.35]} color={skin} /><Segment a={thigh} b={knee} radius={limb * 1.12} color={skin} /><Segment a={knee} b={calf} radius={limb * .94} color={skin} /><Ellipsoid position={ankle} scale={[limb * .8, limb * .72, limb * .82]} color={skin} /><Foot position={[ankle[0], ankle[1] - .012 * s, ankle[2] + .025 * s]} color={skin} scale={s * m.limb} /></group>; })}
  </group>
