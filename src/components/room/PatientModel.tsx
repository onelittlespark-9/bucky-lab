...
export function PatientModel() {
  ...
  else { groupPos = [equipment.tableX + equipment.patientX, tableTop + bodyThickness, equipment.tableZ + h * 0.5 + equipment.patientZ]; groupRot = [-Math.PI / 2, 0, yaw]; }
  // Patient landmark coordinates are defined head-to-foot in centimetres. Three.js uses +Y upward, so invert the vertical axis here.
  const y = (cm: number) => h - (cm / patient.heightCm) * h;
  const pelvisY = y(82), waistY = y(69), chestY = y(49), shoulderY = y(31), neckY = y(18); const hipGap = 0.085 * m.hip; const kneeY = y(45), ankleY = y(5), handDrop = 0.34 * m.limb;
  ...
  return <group position={groupPos} rotation={groupRot}>
    ...
    {showLandmarks ? landmarks.map(lm => { const yy = h - (scaleLandmarkY(lm.y, patient.heightCm) / patient.heightCm) * h; const xx = (lm.x / 100) * m.torsoWidth; return <mesh key={lm.id} position={[xx, yy, bodyThickness + 0.01]} onClick={e => { e.stopPropagation(); setLandmarkCR(scaleLandmarkY(lm.y, patient.heightCm), lm.x * m.torsoWidth); }}><sphereGeometry args={[0.012, 10, 8]} /><meshBasicMaterial color="#e2c35a" /></mesh>; }) : null}
  </group>;
}