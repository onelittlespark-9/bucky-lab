...
export function InternalAnatomy(){
 const patientId=useSim(s=>s.patientId),projectionId=useSim(s=>s.projectionId),exposing=useSim(s=>s.exposing),vis=useSim(s=>s.anatomyVisibility),pose=useSim(s=>s.pose),equipment=useSim(s=>s.equipment),patient=patientById(patientId),H=patient.heightCm/100,s=H/1.7,torsoW=.32*patient.morph.torsoWidth*s,torsoD=.24*patient.morph.torsoDepth*s,anatomyOpacity=exposing?.96:.34,lateral=projectionId.includes("lat"),point=(cm:number)=>H-scaleAnatomyCm(cm,patient.heightCm)/100,organScale=(cm:number)=>scaleAnatomyCm(cm,patient.heightCm)/100;
