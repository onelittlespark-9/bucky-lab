...
function Material({ color, opacity }: { color: string; opacity: number }) { return <meshPhysicalMaterial color={color} transparent opacity={opacity} roughness={.62} metalness={0} depthWrite={false} depthTest={false} side={THREE.DoubleSide} />; }
function Organ({ position, scale, color, opacity }: { position: V3; scale: V3; color: string; opacity: number }) { return <mesh position={position} scale={scale} renderOrder={5}><sphereGeometry args={[1, 28, 20]} /><Material color={color} opacity={opacity} /></mesh>; }
function Tube({ a, b, radius, color, opacity }: { a: V3; b: V3; radius: number; color: string; opacity: number }) { const { position, quaternion, length } = useMemo(() => { const start = new THREE.Vector3(...a), end = new THREE.Vector3(...b), d = end.clone().sub(start), length = d.length(), quaternion = new THREE.Quaternion().setFromUnitVectors(new THREE.Vector3(0, 1, 0), d.normalize()); return { position: start.add(end).multiplyScalar(.5), quaternion, length }; }, [a, b]); return <mesh position={position} quaternion={quaternion} renderOrder={6}><capsuleGeometry args={[radius, Math.max(.01, length - radius * 2), 8, 12]} /><Material color={color} opacity={opacity} /></mesh>; }
function Bone({ position, scale, opacity }: { position: V3; scale: V3; opacity: number }) { return <mesh position={position} scale={scale} renderOrder={4}><sphereGeometry args={[1, 20, 12]} /><Material color="#e7dfc9" opacity={opacity} /></mesh>; }
...
export function InternalAnatomy() {
  ...
  const patient = patientById(patientId), H = patient.heightCm / 100, s = H / 1.7, xScale = patient.morph.torsoWidth * s, zScale = patient.morph.torsoDepth * s, opacity = exposing ? .82 : .24;
  const Y = { head: .955 * H, neck: .86 * H, clavicle: H - scaleLandmarkY(26, patient.heightCm) / 100, heart: H - scaleLandmarkY(42, patient.heightCm) / 100, diaphragm: H - scaleLandmarkY(48, patient.heightCm) / 100, stomach: H - scaleLandmarkY(55, patient.heightCm) / 100, liver: H - scaleLandmarkY(56, patient.heightCm) / 100, kidneys: H - scaleLandmarkY(63, patient.heightCm) / 100, pelvis: H - scaleLandmarkY(72, patient.heightCm) / 100, pubis: H - scaleLandmarkY(82, patient.heightCm) / 100 };
  const lungDepth = Math.max(.035, zScale * .62), organDepth = Math.max(.045, zScale * .55), lateral = projectionId.includes("lat"), z = lateral ? .035 * zScale : -.005 * zScale;
  return <group>
    ...
    {Array.from({ length: 18 }, (_, i) => { const y = H - scaleLandmarkY(22 + i * 3.8, patient.heightCm) / 100, lumbar = i > 11; return <Bone key={`v-${i}`} position={[0, y, -.025 * zScale]} scale={[(lumbar ? .027 : .021) * s, .018 * s, .026 * s]} opacity={opacity} />; })}
    {Array.from({ length: 9 }, (_, i) => { const y = H - scaleLandmarkY(28 + i * 2.9, patient.heightCm) / 100, width = (.12 - i * .002) * patient.morph.torsoWidth * s; return <group key={`rib-${i}`}><Tube a={[-.015 * s, y, 0]} b={[-width, y - .006 * s, -.01 * s]} radius={.008 * s} color="#e7dfc9" opacity={opacity * .72} /><Tube a={[.015 * s, y, 0]} b={[width, y - .006 * s, -.01 * s]} radius={.008 * s} color="#e7dfc9" opacity={opacity * .72} /></group>; })}
