...
export function BodyShell() {
  ...
  const showSkin = useSim(s => s.anatomyVisibility?.skin ?? true);
  ...
  const points = useMemo<Vector2[]>(() => [
    new Vector2(width * .56, .40 * H), new Vector2(width * .78, .45 * H),
    new Vector2(width * .92, .53 * H), new Vector2(width * .98, .61 * H),
    new Vector2(width * 1.0, .69 * H), new Vector2(width * 1.02, .75 * H),
    new Vector2(width * .92, .80 * H), new Vector2(width * .48, .855 * H),
  ], [H, width]);
