...
function Joint({ position, radius, opacity }: { position: V3; radius: number; opacity: number }) {
  return (
    <group position={position} renderOrder={11}>
      ...
      <mesh scale={[1.12, 0.72, 1.12]}>
        <sphereGeometry args={[radius, 14, 10]} />
        <meshPhysicalMaterial color="#d7e1d0" transparent opacity={opacity * 0.62} roughness={0.42} depthWrite={false} depthTest={false} />
      </mesh>
    </group>
  );
}
...
export function BoneJointLayer({
  ...
  const bones = useMemo(() => {
    ...
    // Axial skeleton. Coordinates deliberately share the same Y landmarks as PatientModel/InternalAnatomy.
    ...
    ([-1, 1] as const).forEach((side) => {
      ...
      let wrist: V3;
      if (raise > 0.05) {
        upper = [side * (shoulderWidth + 0.055 * s), Y.shoulder + 0.08 * H * raise, 0];
        ...
        upper = [side * (shoulderWidth + 0.055 * s), Y.shoulder - 0.025 * s, 0];
        elbowPoint = [upper[0] + side * 0.005 * s, upper[1] - 0.15 * s * Math.cos(elbow), 0.02 * s * Math.sin(elbow)];
        wrist = [elbowPoint[0] + side * 0.012 * s, elbowPoint[1] - 0.15 * s * Math.cos(elbow), 0.04 * s * Math.sin(elbow)];
      }
      const hand: V3 = [wrist[0] + side * 0.004 * s, wrist[1] - 0.055 * s, wrist[2]];
      out.push({ a: shoulderPoint, b: upper, r: limb * 1.08 });
      out.push({ a: upper, b: elbowPoint, r: limb * 0.82 });
      out.push({ a: elbowPoint, b: wrist, r: limb * 0.68 });
      out.push({ a: wrist, b: hand, r: limb * 0.50 });
      ...
      out.push({ a: hipPoint, b: thigh, r: limb * 1.20 });
      out.push({ a: thigh, b: knee, r: limb * 0.94 });
      out.push({ a: knee, b: calf, r: limb * 0.82 });
      out.push({ a: calf, b: ankle, r: limb * 0.70 });
      out.push({ a: ankle, b: foot, r: limb * 0.52 });
    });
    return out;
  }, [H, s, torsoWidth, torsoDepth, shoulderWidth, hipGap, limb, elbow, hipInternal, raise, Y.neck, Y.shoulder, Y.chest, Y.pelvis, Y.knee, Y.ankle]);

  const joints = useMemo(() => {
    const result: { p: V3; r: number }[] = [
      { p: [0, Y.neck, 0], r: 0.018 * s },
      { p: [0, Y.pelvis, 0], r: 0.035 * s },
    ];
    ([-1, 1] as const).forEach((side) => {
      const shoulderPoint: V3 = [side * shoulderWidth, Y.shoulder, 0];
      const upper: V3 = [side * (shoulderWidth + 0.055 * s), Y.shoulder + 0.08 * H * raise, 0];
      const elbowPoint: V3 = [upper[0] + side * 0.005 * s, upper[1] - 0.16 * s * Math.cos(elbow), 0.02 * s * Math.sin(elbow)];
      const wrist: V3 = [elbowPoint[0] + side * 0.012 * s, elbowPoint[1] - 0.16 * s * Math.cos(elbow), 0.04 * s * Math.sin(elbow)];
      const hipPoint: V3 = [side * hipGap, Y.pelvis - 0.01 * s, 0];
      const thigh: V3 = [side * (hipGap + 0.008 * s), Y.knee + 0.12 * H, side * 0.008 * s * Math.sin(hipInternal)];
      const knee: V3 = [side * (hipGap + 0.006 * s), Y.knee, side * 0.012 * s];
      const calf: V3 = [side * (hipGap + 0.006 * s), Y.ankle + 0.12 * H, side * 0.008 * s];
      const ankle: V3 = [side * (hipGap + 0.006 * s), Y.ankle, side * 0.01 * s];
      result.push({ p: shoulderPoint, r: limb * 1.18 }, { p: elbowPoint, r: limb * 0.92 }, { p: wrist, r: limb * 0.76 }, { p: hipPoint, r: limb * 1.34 }, { p: knee, r: limb * 1.08 }, { p: ankle, r: limb * 0.84 });
    });
    return result;
  }, [H, s, shoulderWidth, hipGap, limb, elbow, hipInternal, raise, Y.neck, Y.shoulder, Y.pelvis, Y.knee, Y.ankle]);
  ...
  return (
    <group>
      ...
      {/* Rib cage: kept on the same local torso coordinate system as the organs. */}
