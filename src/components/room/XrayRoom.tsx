...
function TableAndBucky() {
  ...
  return <>
    ...
    {onTable && <group position={[equipment.tableX, 0, equipment.tableZ]}>
      {([-0.28, .28] as const).map(x => ([-.9, .9] as const).map(z => <mesh key={`leg-${x}-${z}`} position={[x, th * .5, z]}>
        <boxGeometry args={[.06, th, .06]} />
        <meshStandardMaterial color="#3a4148" metalness={.5} roughness={.4} />
      </mesh>))}
      <mesh position={[0, th, 0]} receiveShadow>
        <boxGeometry args={[.78, .05, 2.3]} />
        <meshStandardMaterial color="#6a5f58" roughness={.85} />
      </mesh>
      <mesh position={[0, th - .04, 0]}>
        <boxGeometry args={[.76, .03, 2.27]} />
        <meshStandardMaterial color="#8b9096" metalness={.3} roughness={.45} />
      </mesh>
    </group>}
    {wall && <group position={[0, 0, -.78]}>
      <mesh position={[0, 1.15, 0]}>
        <boxGeometry args={[.12, 2.3, .12]} />
        <meshStandardMaterial color="#3a4148" metalness={.45} roughness={.4} />
      </mesh>
      <group position={[0, equipment.buckyHeight, .1]} rotation={[tilt, 0, 0]}>
        <mesh>
          <boxGeometry args={[.55, .62, .08]} />
          <meshStandardMaterial color="#1c2228" metalness={.3} roughness={.4} />
        </mesh>
        <mesh position={[0, 0, .05]}>
          <boxGeometry args={[.43, .48, .02]} />
          <meshStandardMaterial color="#050608" roughness={.95} />
        </mesh>
      </group>
    </group>}
...
function AnatomyControls() {
  ...
  return <Html position={[1.25, 1.8, 0]} transform={false} style={{ pointerEvents: "auto" }}>
    <div style={{ background: "rgba(10,14,18,.9)", padding: 12, borderRadius: 10, color: "white", fontFamily: "system-ui", fontSize: 12, width: 150 }}>
      <strong>ANATOMY LAYERS</strong>
      {labels.map(([id, label]) => <label key={id} style={{ display: "flex", gap: 8, alignItems: "center", marginTop: 7, cursor: "pointer" }}>
        <input type="checkbox" checked={visibility[id]} onChange={e => setLayer(id, e.target.checked)} />
        {label}
      </label>)}
    </div>
  </Html>;
