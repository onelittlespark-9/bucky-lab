...
function tableTargetZ(tubeCrY: number, patientId: string, tableZ: number, patientZ: number, projectionId: string) {
  ...
  switch (projection.anatomy) {
    ...
    case "ankle-ap": localY = 0.1 * h; break;
    case "knee-ap":
    case "knee-lat": localY = 0.23 * h; break;
    case "hand-pa":
    case "wrist-pa": localY = 0.4 * h; break;
...
function LightField() {
  ...
  const targetZ = tableTargetZ(tube.crY, patientId, equipment.tableZ, equipment.patientZ, projectionId);
  ...
  if (wall) {
    ...
    tubePos = [detX, equipment.buckyHeight, -0.55 + sidM];
  } else {
  ...
  const coneGeo = useMemo(() => {
    ...
    const positions = new Float32Array([-hw0, -hh0, 0, hw0, -hh0, 0, hw0, hh0, 0, -hw0, hh0, 0, -hw1, -hh1, -depth, hw1, -hh1, -depth, hw1, hh1, -depth, -hw1, hh1, -depth]);
...
function TubeHead() {
  ...
  if (wall) {
    const detY = lock ? equipment.buckyHeight : equipment.buckyHeight + (tube.crY / 100 - 1) * 0.15;
    const detX = lock ? 0 : tube.crX / 100;
...
export function XrayRoom() {
  return <><color attach="background" args={["#0a0c0e"]} /><hemisphereLight args={["#c8d0d4", "#1a1814", 0.55]} /><directionalLight position={[2.5, 4, 2]} intensity={1.15} castShadow shadow-mapSize={[1024, 1024]} /><directionalLight position={[-2, 2, -1]} intensity={0.25} /><TableAndBucky /><PatientModel /><TubeHead /><LightField /><ContactShadows opacity={0.35} scale={8} blur={2.2} far={5} /><OrbitControls enablePan minPolarAngle={0.2} maxPolarAngle={Math.PI / 2.05} minDistance={1.2} maxDistance={6} target={[0, 0.9, 0]} /></>;
}