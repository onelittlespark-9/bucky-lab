...
import { learningLabelsForProjection } from "@/lib/sim/radiograph-learning";
...
function improvementSuggestions(objective: ReturnType<typeof evaluatePLATECAANN>) {
  ...
  for (const item of objective.items) {
    ...
    if (item.key === "T") suggestions.push(item.note);
    else if (item.key === "E-contrast") suggestions.push("Adjust the acquisition technique only if required; the target is clinically useful attenuation and grey-scale differentiation, not simply higher display contrast.");
    ...
    else if (item.key === "C" || item.key === "A-area") suggestions.push(item.note);
...
export function RadiographViewer() {
  ...
  const labels = useMemo(() => learningLabelsForProjection(projection), [projection]);
  ...
  if (!result) {
    return <div className="flex h-full items-center justify-center text-sm text-muted">No image. Return to the room and expose.</div>;
  }

  const { metrics } = result;
  const repeatChoice = plate.find(x => x.key === "N-repeat")?.decision;
  const furtherChoice = plate.find(x => x.key === "N-further")?.decision;
  const canSubmit = critiqueUnlocked && plate.every(x => x.decision !== "concern") && Boolean(findings.trim() && impression.trim());
  const setDecision = (key: PlatecaannItem["key"], decision: PlatecaannDecision) => setPlate(items => items.map(item => (item.key === key ? { ...item, decision } : item)));
  ...
  return (
    <div className="grid h-full min-h-0 grid-cols-1 lg:grid-cols-[minmax(0,1.1fr)_minmax(360px,0.9fr)]">
      <div className="flex min-h-64 flex-col bg-well p-3">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2 rounded-md border border-border bg-surface p-2">
          <div className="flex items-center gap-2">
            ...
            {labels.length > 0 && <Badge tone={learningMode ? "ok" : undefined}>Learning labels {learningMode ? "on" : "off"}</Badge>}
          </div>
          <div className="flex flex-wrap gap-2">
            {labels.length > 0 && <Button size="sm" variant={learningMode ? "solid" : "outline"} onClick={() => setLearningMode(v => !v)}>{learningMode ? "Hide anatomy labels" : "Learning mode"}</Button>}
        ...
        <div className="min-h-0 flex-1 overflow-auto">
          <div className="flex min-h-full min-w-full items-center justify-center p-4">
            <div className="relative inline-block max-h-full max-w-full origin-center transition-transform" style={{ transform:`scale(${zoom})` }}>
              <img src={result.dataUrl} alt={`${projection.shortName} radiograph of ${patient.name}`} className="block max-h-full max-w-full object-contain shadow-[0_0_40px_rgba(0,0,0,0.5)]" style={{ filter:`brightness(${brightness}) contrast(${contrast})` }} />
              {learningMode && labels.map(label => (
                <div key={label.id} className="pointer-events-none absolute -translate-x-1/2 -translate-y-1/2" style={{left:`${label.x*100}%`,top:`${label.y*100}%`}}>
                  <span className="block h-2 w-2 rounded-full border border-white bg-black/70 shadow" />
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 whitespace-nowrap rounded border border-white/60 bg-black/80 px-1.5 py-0.5 text-[10px] font-medium text-white shadow-lg">{label.label}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <aside className="flex min-h-0 flex-col gap-4 overflow-y-auto border-t border-border bg-surface p-4 lg:border-l lg:border-t-0">
        <div>
          <p className="text-xs text-muted">{patient.name} · {projection.shortName}{request ? ` · view ${viewIndex + 1}/${request.requestedProjections.length}` : ""}</p>
          <h2 className="text-base font-medium">{critiqueUnlocked ? "Final-image PLATECAANN critique" : "Automatic image assessment"}</h2>
          <p className="mt-1 text-xs text-muted">{critiqueUnlocked ? "Critique the final image yourself. Do not equate every imperfection with a repeat; decide whether the defect compromises the clinical question." : "The simulator has assessed the exposed image first. Use the projection-specific feedback to decide whether the image should be improved before you perform your own critique."}</p>
        </div>

        <div className="rounded-md border border-border bg-elevated p-4">
          <div className="flex items-center justify-between gap-3">
            <div><p className="text-[11px] uppercase tracking-wider text-muted">Automatic image decision score</p><p className={`mt-1 font-mono text-3xl font-semibold ${scoreTone === "danger" ? "text-danger" : scoreTone === "warn" ? "text-warn" : "text-ok"}`}>{objective?.score ?? 0}/100</p></div>
        ...
        {!critiqueUnlocked && objective && objective.detailedCritique.length > 0 ? (
          <div className="rounded-md border border-border bg-elevated p-4">
            <h3 className="text-sm font-semibold">Detailed projection critique</h3>
            <p className="mt-1 text-xs text-muted">These observations describe the anatomy and attenuation expected on the selected projection, not just whether the exposure controls matched a preset.</p>
            <div className="mt-3 space-y-3">
              {objective.detailedCritique.map(c => (
                <div key={c.id} className="border-t border-border pt-2 first:border-t-0 first:pt-0">
                  <div className="flex items-center justify-between gap-2"><span className="text-xs font-semibold text-fg">{c.heading}</span><Badge tone={c.severity === "good" ? "ok" : c.severity === "review" ? "warn" : "danger"}>{c.severity === "good" ? "Good" : c.severity === "review" ? "Review" : "Repeat-level"}</Badge></div>
                  <p className="mt-1 text-xs leading-relaxed text-muted"><strong className="text-fg">Observed:</strong> {c.observation}</p>
                  <p className="mt-1 text-xs leading-relaxed text-muted"><strong className="text-fg">Expected:</strong> {c.expected}</p>
                  {c.correction && <p className="mt-1 text-xs leading-relaxed text-muted"><strong className="text-fg">Correction:</strong> {c.correction}</p>}
                </div>
              ))}
            </div>
          </div>
        ...
        {!critiqueUnlocked ? (
          <div className="rounded-md border border-border bg-elevated p-4">
            ...
            <ul className="mt-2 space-y-2 text-xs leading-relaxed text-muted">{suggestions.map((s, i) => <li key={i}>• {s}</li>)}</ul>
            <p className="mt-3 text-xs text-muted">If a repeat is needed, return to the room and correct the acquisition. If the image is acceptable, continue without repeating.</p>
            <div className="mt-3 grid gap-2">
              <Button variant="solid" onClick={() => setCritiqueUnlocked(true)}>Begin final-image critique</Button>
              {objective?.repeatRequired && <Button variant="outline" onClick={retake}>Return to room and improve image</Button>}
            </div>
          </div>
        ) : null}
        ...
        {critiqueUnlocked && (
          <>
              ...
              {plate.map(item => (
                <div key={item.key} className="rounded-md border border-border bg-elevated p-3">
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      <Badge>{item.key.replace("E-", "E — ").replace("A-area", "A").replace("A-artifact", "A").replace("A-abnormality", "A").replace("N-repeat", "N").replace("N-further", "N")}</Badge>
                      <span className="text-sm font-medium">{item.label}</span>
                    </div>
                    <select value={item.decision} onChange={e => setDecision(item.key, e.target.value as PlatecaannDecision)} className="rounded border border-border bg-surface px-2 py-1 text-xs text-fg">
                      <option value="concern">Review</option><option value="pass">Pass</option><option value="fail">Fail</option>
                    </select>
                  </div>
            ...
            <div className="rounded-md border border-border bg-elevated p-3 text-xs">
              ...
              <p className="mt-1 text-muted">Automatic score above is based on the simulated acquisition. Your task now is to independently judge the final image.</p>
              {repeatChoice !== "concern" && <p className="mt-1">Repeat decision: <strong>{repeatChoice === "fail" ? "REPEAT" : "ACCEPT"}</strong> {repeatCorrect ? "— correct" : "— review"}</p>}
              {furtherChoice !== "concern" && <p>Further views: <strong>{furtherChoice === "fail" ? "REQUIRED" : "NOT REQUIRED"}</strong> {furtherCorrect ? "— correct" : "— review"}</p>}
            </div>
            <Separator />
            <div className="space-y-2">
              <h3 className="text-sm font-medium">Your radiographic findings</h3>
              <textarea value={findings} onChange={e => setFindings(e.target.value)} placeholder="Describe the abnormality, location and relevant radiographic signs…" className="min-h-24 w-full rounded-md border border-border bg-elevated p-2 text-sm text-fg outline-none" />
              <textarea value={impression} onChange={e => setImpression(e.target.value)} placeholder="Your impression…" className="min-h-16 w-full rounded-md border border-border bg-elevated p-2 text-sm text-fg outline-none" />
            </div>
            {submitted && report ? (
              <>
                <div className="rounded-md border border-border bg-elevated p-4">
                  <div className="flex items-center justify-between"><h3 className="text-sm font-semibold">PACS — Radiologist-style reference report</h3><Badge tone="ok">Submitted</Badge></div>
                  <div className="space-y-2 text-xs leading-relaxed">
                    <p><strong>Clinical details:</strong> {report.clinicalHistory}</p><p><strong>Technique:</strong> {report.technique}</p><p><strong>Findings:</strong> {report.findings}</p><p><strong>Impression:</strong> {report.impression}</p>
                  </div>
                </div>
                <div className="rounded-md border border-border bg-elevated p-4">
                  <h3 className="text-sm font-semibold">Clinical decision feedback</h3>
                  <p className="mt-1 text-xs text-muted">Repeat decision: {repeatCorrect ? "Correct" : "Review your decision"}. Further-view decision: {furtherCorrect ? "Correct" : "Review the clinical indication and requested views"}.</p>
                  <p className="mt-2 text-xs text-muted">{objective?.summary}</p>
                </div>
                <div className="border-t border-border pt-3"><p className="text-[11px] uppercase tracking-wider text-muted">Learning comparison</p><p className="mt-1 text-xs text-muted">Compare your findings and impression with the reference report. The aim is to recognise the same clinically important abnormality, not reproduce exact wording.</p></div>
              </>
            ) : null}
          </>
        )}
        ...
        <div className="mt-auto grid gap-2 pt-2">
          {critiqueUnlocked && !submitted ? <Button variant="solid" disabled={!canSubmit} onClick={() => setSubmitted(true)}>Submit image to PACS</Button> : null}
          {hasAnother ? <Button variant="solid" onClick={nextRequestedView}>Take next requested view</Button> : null}
          <div className="grid grid-cols-2 gap-2"><Button variant="outline" onClick={retake}>Repeat this view</Button><Button variant="outline" onClick={() => setScreen("room")}>Return to room</Button></div>
          {submitted && <Button variant={lastView ? "solid" : "outline"} disabled={!lastView} onClick={completeCurrentRequest}>{lastView ? "Complete request" : "Complete after all views"}</Button>}
        </div>
      </aside>
    </div>
  );
...
function Stat({ k, v, tone }: { k: string; v: string; tone?: "ok" | "warn" | "danger" }) {
  return <div><p className="text-[11px] uppercase tracking-wider text-muted">{k}</p><p className={`font-mono text-sm tabular-nums ${tone === "danger" ? "text-danger" : tone === "warn" ? "text-warn" : tone === "ok" ? "text-ok" : "text-fg"}`}>{v}</p></div>;
}
