...
export function SetupScreen() {
  ...
  const confirmSetup = useSim(s => s.confirmSetup); const patchPose = useSim(s => s.patchPose); const setScreen = useSim(s => s.setScreen); const patchEquipment = useSim(s => s.patchEquipment);
  ...
  const enterRoom = () => {
    ...
    confirmSetup(selected); patchPose(poseForExtremityPlacement(projectionId, selected, equipment.buckyTilt, pose));
  };

  return <div className="flex min-h-dvh flex-col bg-bg">
    <header className="border-b border-border bg-surface px-4 py-3"><p className="text-xs text-muted">Patient examination · request review</p><h1 className="text-lg font-semibold text-fg">{projection.name}</h1><p className="text-sm text-muted">{patient.name} · {patient.age} years · {patient.habitus}</p></header>
    <main className="mx-auto flex w-full max-w-2xl flex-1 flex-col gap-4 p-4">
      {specificity && <section className="rounded-xl border border-border bg-surface p-4">
        ...
        {!interviewStarted ? <Button className="mt-4" onClick={() => setInterviewStarted(true)}>Ask: “Can you tell me why you're here today?”</Button> : <div className="mt-4 space-y-3">
          ...
          {patientConfirmed === false && <div className="rounded-lg border border-red-500/30 bg-red-500/5 p-3 text-sm"><strong className="text-fg">Stop before exposure.</strong><p className="mt-1 text-muted">The patient's answer does not confirm the request. Query the request rather than choosing an AOI yourself.</p>{discrepancy && <p className="mt-2 font-medium text-fg">This scenario is deliberately inconsistent: the referral says left, but the patient reports the right side.</p>}<Button size="sm" variant="outline" className="mt-3" onClick={() => setScreen("library")}>Return to worklist / query request</Button></div>}
      ...
      <div className="mt-auto flex flex-col gap-2 pt-2"><Button variant="solid" size="lg" className="w-full" disabled={!interviewStarted || patientConfirmed !== true || !aoiConfirmed || (needsSide && !sideChoice)} onClick={enterRoom}>Continue to positioning</Button><Button variant="ghost" className="w-full" onClick={() => setScreen("library")}>Back to worklist</Button></div>
    </main>
  </div>;
}
