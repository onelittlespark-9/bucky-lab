...
const RoomViewport = lazy(() =>
  import("@/components/room/RoomViewport").then((m) => ({ default: m.RoomViewport })),
);

function RoomFallback() {
  return (
    <div className="flex h-full min-h-64 items-center justify-center bg-bg text-sm text-muted">
      Loading the room…
    </div>
  );
}
  ...
  return (
    <div className="flex h-dvh min-h-0 flex-col">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(300px,360px)]">
        <div className="min-h-72 border-b border-border lg:border-b-0 lg:border-r">
          {mounted ? (
        ...
        <div className="min-h-0 overflow-hidden bg-panel p-4">
          <ControlDeck />
        </div>
      </div>
    </div>
...
function TopBar() {
  ...
  return (
    <header className="flex items-center gap-3 border-b border-border bg-surface px-3 py-2">
      <Button variant="ghost" size="sm" onClick={() => setScreen("library")} className="gap-1.5">
        <ArrowLeft className="size-4" />
        Library
      </Button>
      <div className="min-w-0 flex-1">
        <p className="truncate text-sm font-medium">{projection.shortName}</p>
        <p className="truncate text-[11px] text-muted">
          {patient.name} · {patient.habitus} · {patient.heightCm} cm
        </p>
      </div>
      <Badge>{projection.setup}</Badge>
    </header>
  );
}
