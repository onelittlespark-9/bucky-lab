    ...
    <main className="min-h-dvh bg-bg px-5 py-8 text-fg sm:px-8">
      <div className="mx-auto flex min-h-[calc(100dvh-4rem)] max-w-5xl flex-col justify-center">
        <header className="mx-auto mb-9 max-w-2xl text-center">
          ...
          <p className="mx-auto mt-4 max-w-xl text-sm leading-6 text-muted">Choose how you want to use the laboratory.</p>
        </header>

        <section className="grid gap-4 md:grid-cols-3">
          <button type="button" onClick={() => { setUsername(""); setError(""); setShowTestGate(true); }} className="group rounded-2xl border border-border bg-surface p-6 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-accent/50 hover:shadow-lg">
          ...
          <Link to="/cases" className="group rounded-2xl border border-border bg-surface p-6 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-accent/50 hover:shadow-lg">
          ...
          <Link to="/critique-library" className="group rounded-2xl border border-border bg-surface p-6 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-accent/50 hover:shadow-lg">
      ...
      {showTestGate ? (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/65 p-4" role="dialog" aria-modal="true" aria-labelledby="test-access-title">
          <form onSubmit={submitTestAccess} className="w-full max-w-sm rounded-2xl border border-border bg-surface p-6 shadow-2xl">
            ...
            <input id="test-username" autoFocus value={username} onChange={e => { setUsername(e.target.value); setError(""); }} className="mt-2 h-11 w-full rounded-lg border border-border bg-bg px-3 text-sm text-fg outline-none focus:border-accent" autoComplete="off" />
